...
SysMenuApplet *
menu_get_applet (GtkWidget *menu)
{
  SysMenuApplet *retval = NULL;
  ...
  while (menu) {
    retval = g_object_get_data (G_OBJECT (menu), "menu_applet");
    if (retval)
      break;
  ...
  if (!retval) {
    g_warning ("Cannot find the SysMenuApplet associated with menu");
  }
...
static void
setup_menu_applet (GtkWidget *menu)
{
  SysMenuApplet *applet;

  applet = g_object_get_data (G_OBJECT (menu), "menu_applet");
  if (applet)
    return;

  applet = menu_get_applet (menu);
  g_object_set_data (G_OBJECT (menu), "menu_applet", applet);
...
menuitem_to_screen (GtkWidget *menuitem)
{
  ...
  applet = menu_get_applet (menuitem);

  return gtk_widget_get_screen (GTK_WIDGET (applet->applet));
}
...
create_empty_menu (void)
{
  ...
  g_signal_connect (retval, "show", G_CALLBACK (setup_menu_applet), NULL);
		 ...
		 GMenuTreeDirectory *alias_directory)
{
  ...
  if (! /*panel_lockdown_get_panels_locked_down_s ()*/FALSE) {
    GIcon *icon;
